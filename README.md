...
### 最新变动 (Neo 1.8 alpha)
* **数值调整 / Stats**: 重新设计了冷战武器部件和船体的数值。
* **技术扩展 / Tech**: 延长了火控、损管和登陆科技树。
* **布局优化 / Layout**: 调整了海军支援武器的布局。
* **新增内容 / New Content**: 添加了两栖登陆舰 (Amphibious Assault Ships)。
* **美术优化 / Visuals**: 补全并优化了部分纹理。
* **界面重构 / UI**: 重新设计了舰船建造界面，支持显示过量舰船类型。
* **本地化 / Loc**: 修复了英文模式下 VNR 分类显示中文的问题。
* **其他 / Misc**: 在 VNR 介绍界面添加了模组 Label。
